    ...
    do {
        ...
        switch (opcion) {
            case 1: {
                // Crear hilos
                for (int i = 0; i < 4; i++) {
                    if (cethread_create(&hilos[i], rutina, (void *)&procesos[i])) { // Instead of if (pthread_create(&hilos[i], NULL, rutina, (void *)&procesos[i])) {
                        fprintf(stderr, "Error al crear el hilo %d\n", i);
                        return 1;
                    }
                    // Esperar a que el hilo actual termine antes de crear el siguiente
                    cethread_join(hilos[i], NULL); // Instead of pthread_join(hilos[i], NULL);
                }
                break;
            }
            case 2: {
